...
class AnthropicLLMClient(LLMClient):
    ...
    def generate_quiz(self, source_text: str, title: str, lang: str = "fr") -> list[dict]:
        raw = self._call_anthropic(source_text, title, lang)
        return parse_and_validate_quiz(raw)

    # ----- internals -----

    def _call_anthropic(self, source_text: str, title: str, lang: str) -> str:
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "system": build_system_prompt(lang),  # consignes isolées du contenu utilisateur
                    "messages": [
                        {"role": "user", "content": build_user_prompt(source_text, title, lang)},
                    ],
                    "temperature": 0.4,
                },
