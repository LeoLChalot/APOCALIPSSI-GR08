...
class MockLLMClient(LLMClient):
    ...
    def generate_quiz(self, source_text: str, title: str, lang: str = "fr") -> list[dict]:
        ...
        for i in range(1, 11):
            ...
            if is_english:
                options = [
                    f"Answer A about '{word}'",
                    f"Answer B about '{word}'",
                    f"Answer C about '{word}'",
                    f"Answer D about '{word}'",
                ]
                options[correct_idx] = f"✓ Mock correct answer for '{word}' (question {i})"
                prompt = f"[MOCK Q{i}] Based on the course “{title}”, which statement about “{word}” is correct?"
            else:
                options = [
