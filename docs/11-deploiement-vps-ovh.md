...
> ⚠️ **À lire avant tout.** Le Kit `IPSSI_APOCAL_KIT` est conçu pour le **développement local** : le frontend tourne via `npm run dev` (serveur Vite), le backend via `manage.py runserver`, et les quatre services (postgres, ollama, backend, frontend) exposent leurs ports directement sur l'hôte. **Rien de tout cela n'est acceptable sur une machine publique.** Ce guide bascule le Kit en configuration de **production** sur un VPS OVHcloud (Ubuntu 24.04 LTS) : reverse proxy Caddy en HTTPS, frontend buildé en statique, backend servi par gunicorn, base PostgreSQL, LLM cloud par défaut, et **aucun port applicatif exposé sur Internet** (seul Caddy écoute sur 80/443).
...
4. **Récupérer le code & créer le `.env` de production** — cloner le dépôt, partir de `.env.prod.example`, générer des secrets forts, `DJANGO_DEBUG=False`, `.env` en `600` et hors Git.
...
| RAM requise | 2–4 Go suffisent | **8 Go minimum**, 16 Go confortable (modèle + Postgres/Django/front) |
| Disque | 20–40 Go | **+2 à 5 Go pour le modèle** selon celui choisi (`llama3.2:3b` ≈ 2 Go) |
...
> ⚠️ **Les VPS OVH n'ont pas de GPU.** Faire tourner `llama3.2:3b` en CPU pur peut déjà rester lent (souvent 1 à 3 min par génération) et saturer fortement les cœurs, ce qui bloque les autres services pendant la génération. **Pour une démo fluide en formation, choisissez un backend cloud free tier (Groq ou Mistral) et un petit VPS.** Ne partez sur Ollama local que si la souveraineté/le hors-ligne est une exigence explicite, et dans ce cas dimensionnez large. La stratégie LLM détaillée fait l'objet de la **section 8**.
...
- **4 vCore / 16 Go RAM / 160 Go SSD-NVMe** recommandé : confortable si vous voulez garder de la marge ou tester ensuite un modèle plus lourd qu'un `llama3.2:3b`.
...
Cette étape récupère le code du Kit sur le VPS et construit un fichier `.env` **de production** propre, dérivé de `.env.prod.example`. Les commandes sont à exécuter en tant qu'utilisateur non-root `<USER>`.

> ⚠️ **Principe de base.** Les modèles `.env.example` et `.env.prod.example` fournis dans le dépôt sont désormais **sans secrets réels**. En revanche, si votre fork a déjà contenu une vraie clé API, une vraie clé SMTP ou un mot de passe committé, considérez-le comme **compromis** : révoquez-le, scannez l'historique Git et purgez-le si nécessaire (voir 4.6).
...
cd IPSSI_APOCAL_KIT
ls -1 docker-compose.yml .env.example .env.prod.example Makefile
```
...
> ℹ️ **Convention de chemin pour tout le guide.** On suppose désormais le projet directement dans `/home/<USER>/IPSSI_APOCAL_KIT`, avec `docker-compose.yml`, `.env` et `Makefile` à cette racine. Toutes les commandes des sections suivantes s'exécutent depuis cette racine du dépôt.
...
On part du modèle fourni (ne modifiez pas `.env.prod.example`, il reste versionné sans secrets), puis on génère les secrets nécessaires :
...
> ⚠️ **Le piège des placeholders de prod.** Le `.env` fraîchement copié hérite des valeurs de `.env.prod.example` : domaine d'exemple, secret Django placeholder, mot de passe PostgreSQL placeholder et clés API vides. Si vous ne les écrasez pas un par un comme ci-dessus, vous déployez une config incomplète ou dangereuse.
...
### 4.6. Scanner et purger tout secret éventuellement committé

Retirer une clé du `.env` courant ne suffit pas : si elle a déjà été committée, elle reste dans l'**historique Git**. Trois actions, dans l'ordre :

**1. Révoquer (le plus urgent).** Connectez-vous chez le fournisseur concerné (Brevo, Groq, Mistral, etc.), supprimez/désactivez la clé exposée et générez-en une nouvelle. La révocation rend la clé inopérante, même si elle reste lisible quelque part.

**2. Scanner l'historique Git** pour recenser tous les secrets exposés (clé SMTP, clé API, mot de passe, etc.) avec un outil dédié, par exemple `gitleaks` :
...
> 2. Toute clé réellement committée doit être **révoquée**, et idéalement **purgée de l'historique** (la retirer du fichier courant ne suffit pas).
...
Tous les fichiers ci-dessous se créent à la racine du dépôt (`/home/<USER>/IPSSI_APOCAL_KIT`).
...
> ⚠️ **Contexte de build.** Le `COPY docker/nginx.conf ...` impose que le **contexte de build soit la racine du dépôt** (et non `./frontend`), pour que le dossier `docker/` soit visible. Le `docker-compose.prod.yml` ci-dessous fixe donc `context: .` pour le frontend. Conséquence : ajoutez un `.dockerignore` à la racine (étape 6.6), sinon `node_modules` et `dist` de l'hôte seraient copiés dans le contexte.
...
Créez `docker-compose.prod.yml` à la racine du dépôt :
...
services:
  ...
  # Contexte de build = racine du dépôt (pour accéder à docker/nginx.conf).
  # --------------------------------------------------------------------------
  frontend:
    restart: unless-stopped
    build:
...
> ⚠️ **Le `Caddyfile` est créé à la section 7.** Ce fichier le référence (`./Caddyfile`) : il doit exister à la racine du dépôt avant le `up`, sinon Caddy ne démarrera pas.

### 6.6. Ajouter un `.dockerignore` à la racine

Comme le contexte de build du frontend devient la racine du dépôt, empêchez la copie de gros dossiers inutiles (qui ralentiraient le build et pourraient écraser le `dist/` propre généré dans l'image). Créez `.dockerignore` à la racine du dépôt :
...
| `.dockerignore` | créé | allège le contexte de build (racine du dépôt) |
...
Caddy route `/api/*`, `/admin/*`, `/static/*`, `/media/*` vers le **backend gunicorn** (`backend:8000`), et tout le reste (`/`) vers le **frontend statique** (`frontend:80`). Comme WhiteNoise est activé (section 5.1), `backend:8000` sert correctement `/static/`. Créez le fichier `Caddyfile` à la racine du dépôt (à côté de `docker-compose.prod.yml`) :
...
Sur un VPS d'entrée/milieu de gamme (CPU uniquement, **pas de GPU**), faire tourner `llama3.2:3b` pose déjà trois problèmes concrets :
...
> ⚠️ Ne mettez **jamais** une vraie clé API dans `.env.example`, `.env.prod.example` ni dans aucun fichier versionné : le dépôt `IPSSI_APOCAL_KIT` est **public**. Toute clé committée doit être considérée comme compromise et révoquée (cf. section 4.6).
...
[ ] Dépôt cloné dans /home/<USER>/IPSSI_APOCAL_KIT (racine du dépôt)
...
Fichiers du dépôt vérifiés pour cette version (chemins relatifs à la racine du dépôt) :
